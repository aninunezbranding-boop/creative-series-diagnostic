...
function AccessGate({ onAccess }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#2D0A1F", display:"flex", alignItems:"center", justifyContent:"center", padding:"24px", fontFamily:"'DM Sans', system-ui, sans-serif" }}>
      <div style={{ background:"#FFFFFF", borderRadius:20, padding:"48px 40px", maxWidth:480, width:"100%", textAlign:"center" }}>
        ...
        <p style={{ marginTop:24, fontSize:12, color:"#8C8589" }}>
          ¿No tienes un código?{" "}
          <a href="https://instagram.com/aninunez.branding" style={{ color:"#7B2D52", fontWeight:600 }}>Obtén acceso aquí</a>
        </p>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const s = {
    wrap: { minHeight:"100vh", background:"#F7F4F1", fontFamily:"'DM Sans', system-ui, sans-serif", color:"#1C1018" },
...
function downloadPDF(result) {
  ...
  const html=`<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;600;700&display=swap" rel="stylesheet"><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:'DM Sans',sans-serif;color:#1C1018;background:white;padding:36px;font-size:13px;line-height:1.6}h1,h2,h3{font-family:Georgia,serif;margin-bottom:8px}h1{font-size:2rem}h2{font-size:1.35rem}h3{font-size:1.1rem}.hdr{background:#2D0A1F;color:#F7F4F1;padding:28px 32px;border-radius:16px;margin-bottom:24px}.gbadge{background:rgba(255,255,255,.15);color:#EDE5E9;font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;padding:4px 12px;border-radius:40px;display:inline-block;margin-bottom:10px}.date{color:rgba(247,244,241,.6);font-size:12px;margin-top:4px}.card{border:1px solid #D4C4CE;border-radius:12px;padding:18px;margin-bottom:12px;page-break-inside:avoid}.card.star{border:2px solid #4A1535}.ey{font-size:10px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:#7B2D52;margin-bottom:8px}.sub{color:#8C8589;font-style:italic;margin-bottom:8px}.tags{display:flex;flex-wrap:wrap;gap:6px;margin:8px 0}.tag{background:#2D0A1F;color:#F7F4F1;padding:3px 11px;border-radius:40px;font-size:11px;font-weight:600}.tag-s{border:1.5px solid #4A1535;color:#4A1535;padding:3px 11px;border-radius:40px;font-size:11px;font-weight:600}.alert{background:#F2EAF0;border-left:3px solid #4A1535;padding:9px 12px;border-radius:6px;margin-top:10px;font-size:12px}.meta{display:flex;gap:8px;margin:10px 0}.mc{flex:1;background:#F2EAF0;border-radius:8px;padding:9px;text-align:center;font-size:12px;font-weight:600}.ml{font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:#8C8589;margin-bottom:3px}.ml2{font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:#4A1535;margin-bottom:3px}.ep{background:#EDE5E9;border-radius:8px;padding:9px 11px;margin-bottom:10px;font-size:12px}.why{font-size:12px;color:#6B5A63;border-top:1px solid #D4C4CE;padding-top:8px}.ang{background:#F2EAF0;border-left:3px solid #4A1535;border-radius:8px;padding:11px 13px;margin-bottom:8px}.dark{background:#2D0A1F;color:#F7F4F1;border-radius:12px;padding:20px;margin-top:8px}.paso{display:flex;gap:10px;align-items:flex-start;margin-bottom:9px;font-size:12px}.pn{width:20px;height:20px;border-radius:50%;background:#C9A0BC;color:#2D0A1F;font-size:10px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0}.ft{margin-top:28px;padding-top:14px;border-top:1px solid #D4C4CE;font-size:11px;color:#8C8589;text-align:center}@media print{body{padding:18px}}</style></head><body><div class="hdr"><div class="gbadge">✓ Creative Series Blueprint</div><h1>${p.esencia||''}</h1><div class="date">Generado el ${new Date().toLocaleDateString('es-ES',{day:'numeric',month:'long',year:'numeric'})}</div></div><div class="card"><div class="ey">Perfil de marca personal</div><h2>${p.esencia||''}</h2><p style="margin-bottom:10px">${p.posicionamiento||''}</p><div class="tags">${(p.tono_dominante||[]).map(t=>`<span class="tag">${t}</span>`).join('')}${(p.tono_secundario||[]).map(t=>`<span class="tag-s">${t}</span>`).join('')}</div>${p.advertencia?`<div class="alert">${p.advertencia}</div>`:''}</div><div class="card"><div class="ey">Ángulos únicos</div><h2>Desde dónde hablas que nadie más puede</h2>${angulos.map(a=>`<div class="ang"><b>${a.nombre}</b><div style="font-size:11px;color:#8C8589;font-style:italic;margin:3px 0">${a.tension}</div><div style="font-size:11px;color:#6B5A63">${a.descripcion}</div></div>`).join('')}</div>${seriesHTML}<div class="dark"><div style="font-size:9px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:rgba(247,244,241,.45);margin-bottom:10px">Próximos pasos</div><h3 style="color:#F7F4F1;margin-bottom:12px">¿Por dónde empezar?</h3>${pasos.map((p2,i)=>`<div class="paso"><div class="pn">${i+1}</div><div>${p2}</div></div>`).join('')}</div><div class="ft">Ani Núñez · Personalidad Rentable</div></body></html>`;
